<template>
  <div class="">
    ...
    <!-- <button aria-label="Color Mode" class="inline-block" @click="onClick">
        <Icon v-if="colorMode.value === 'dark'" name="heroicons-outline:moon" size="16" />
        <input v-if="colorMode.value === 'dark'" type="checkbox" class="toggle" />        <Icon v-else name="heroicons-outline:sun" size="16" />

        <input v-else type="checkbox" class="toggle" checked />
              </button> -->

        <label class="swap swap-rotate btn-ghost">
  <!-- this hidden checkbox controls the state -->
  <input type="checkbox" @change="onClick"/>
  
  <!-- sun icon -->
  <Icon name="material-symbols:sunny-outline" class="swap-on fill-current w-8 h-8" />
